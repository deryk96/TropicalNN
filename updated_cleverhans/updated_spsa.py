...
def spsa(
    ...
    optimizer = SPSAAdam(
        learning_rate=learning_rate, delta=delta, num_samples=spsa_samples, num_iters=spsa_iters
    )
...
class SPSAAdam(tf.optimizers.Adam):
    ...
    def __init__(
        ...
        super(SPSAAdam, self).__init__(learning_rate=learning_rate)
    ...
    def _apply_gradients(self, grads, x, optim_state):
        ...
        for i in range(len(x)):
            ...
            new_x[i] = x[i] - self.learning_rate * m_hat / (tf.sqrt(u_hat) + self.epsilon)
